...
class Gallery extends Component {
  ...
  onChangeImageId = id => this.setState({activeImageId: id})

  render() {
    const {activeImageId} = this.state
    const activeImageDetails = imagesList.filter(
      each => each.id === activeImageId,
    )
    const {imageUrl, imageAltText} = activeImageDetails[0]
    return (
      <div className="bg-container">
        <div className="container">
          <div className="image-container">
            ...
            <p className="para2">
              Nature Photography by <span>Balu</span>
            </p>
            <ul className="thumbnail-items-container">
              {imagesList.map(each => (
                <ThumbnailItem
                  ...
                  imageObject={each}
                  onChangeImageId={this.onChangeImageId}
                  key={each.id}
                />
              ))}
            </ul>
